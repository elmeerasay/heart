...
async function swapARTtoUSDT() {
    try {
        ...
        const amountIn = ethers.utils.parseUnits("4", 18); // 1 ART
        ...
        // Send transaction
        console.log("Sending transaction to swap 4 ART for USDT...");
        ...
        await tx.wait();
        console.log("\x1b[32m%s\x1b[0m", "Transaction confirmed.\n");
    } catch (error) {
        console.error("\x1b[31m%s\x1b[0m", "Saldo Tidak cukup"); //, error);
    }
}
...
async function swapUSDTtoART() {
    try {
        ...
        const amountIn = ethers.utils.parseUnits("500", 18); // 100 USDT
        const to = wallet.address; // Recipient address
        const deadline = Math.floor(Date.now() / 1000) + 60 * 20; // 20 minutes from now
        ...
        // Send transaction
        console.log("Sending transaction to swap 500 USDT for ART...");
